...
def plot_fig1_subplot(title, metric, number, X, results_zero_c, results_one_c):
    ...
    # amod - Hexagon
    Y = results_one_c[np.where(np.logical_and(results_one_c['activation_function'] == b'amod',
                                              results_one_c['information_metric'] == metric))]['value']
    ax.plot(X, Y, color='k', marker='H', ls=' ',  label="amod")

    # max - stars
    Y = results_one_c[np.where(np.logical_and(results_one_c['activation_function'] == b'max',
                                              results_one_c['information_metric'] == metric))]['value']
    ax.plot(X, Y, color='k', marker='*', ls=' ', label="max")
...
def _plot_MI_subplot(metric, number, function, X, Y, results):
    ...
    # Subplot 1: Additive
    ax = plt.subplot(3, 2, number, projection='3d')
...
def _plot_MI(title, metric, X, Y, results):
    ...
    # title, metric, number, X, Y, results, color
    _plot_MI_subplot(metric, 4, b'nocontext', X, Y, results)

    # title, metric, number, X, Y, results, color
    _plot_MI_subplot(metric, 5, b'amod', X, Y, results)

    # title, metric, number, X, Y, results, color
    _plot_MI_subplot(metric, 6, b'max', X, Y, results)
...
def plot_pids(X, Y, results):
    ...
    # amod -
    plot_function_pid("Alternative Modulatory", b'amod', X, Y, results)

    # max -
    plot_function_pid("Max", b'max', X, Y, results)
